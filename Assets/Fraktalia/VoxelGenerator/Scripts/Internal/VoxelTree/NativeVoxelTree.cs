...
namespace Fraktalia.VoxelGen
{
	...
	{		
		...
		[NativeDisableContainerSafetyRestriction]
		public FNativeQueue<IntPtr> NodeQueue;
		...
		public void _Initialize(NativeVoxelReservoir reservoir, float rootSize, int subdivisionPower, byte initialID, int MaxCores)
		{
			...
			NativeVoxelNode voxel = new NativeVoxelNode(0, 255, initialID, voxelReservoir.NullAddress, _ROOT, voxelReservoir.NullAddress);
			...
			NodeQueue = new FNativeQueue<IntPtr>(Allocator.Persistent);
		}
		...
		public int _PeekVoxelId(float fX, float fY, float fZ, byte depth, float shrink = 0, int borderValue = 0)
		{
			...
			for (int i = 0; i < depth; i++)
			{
				...
				{
					current = current.GetChildFast(index);
					id = current._voxelID;
				}
		...
		/// <summary>
		/// Returns the voxel ID at a given local target coordinate. [Optimized with ChatGPT]
		/// </summary>
		/// <param name="X">Local Position X</param>
		/// <param name="Y">Local Position Y</param>
		/// <param name="Z">Local Position Z</param>
		/// <param name="depth">Deprecated (is constant 16)</param>
		/// <param name="shrink">Keep as it is.</param>
		/// <param name="borderValue">Value used if lookup coordinate is out of bounds</param>
		/// <returns></returns>
		[BurstCompile]
		public int _PeekVoxelId_InnerCoordinate(int X, int Y, int Z, byte depth, int shrink = 0, int borderValue = 0, int ignoreNeighbors = 0, int shrinkmax = 0)
		{
			int innerwidth_shrinked = NativeVoxelTree.INNERWIDTH - shrinkmax;

			int reducedWidth = NativeVoxelTree.INNERWIDTH - shrink;
			int blockindex = 13 + 
				 (X >= innerwidth_shrinked ? 1 : 0) +
				 (Y >= innerwidth_shrinked ? 3 : 0) +
				 (Z >= innerwidth_shrinked ? 9 : 0) + 
				 (X < shrink ? -1 : 0) +
				 (Y < shrink ? -3 : 0) +
				 (Z < shrink ? -9 : 0);

			int nfX = (X >= NativeVoxelTree.INNERWIDTH ? X - NativeVoxelTree.INNERWIDTH : X) +
					  (X < shrink ? reducedWidth : 0);

			int nfY = (Y >= NativeVoxelTree.INNERWIDTH ? Y - NativeVoxelTree.INNERWIDTH : Y) +
					  (Y < shrink ? reducedWidth : 0);

			int nfZ = (Z >= NativeVoxelTree.INNERWIDTH ? Z - NativeVoxelTree.INNERWIDTH : Z) +
					  (Z < shrink ? reducedWidth : 0);

			if (blockindex != 13)
			{
				if (ignoreNeighbors == 1) return borderValue;
				IntPtr ptr = Neighbors[blockindex];
				if (ptr == IntPtr.Zero)
				{
					return borderValue;
				...
				ref NativeVoxelTree next = ref UnsafeUtility.AsRef<NativeVoxelTree>(ptr.ToPointer());
			...
			for (int i = 0; i < MaxDepth; i++)
			{
				int index = 0;
				nodeSize = nodeSize >> bitshift;
				int shift = 16 - ((i + 1) << bitmultiply);
				int fractionedX = (X >> shift);
				int fractionedY = (Y >> shift);
				int fractionedZ = (Z >> shift);
		...
		public void UpwardMerge(NativeVoxelNode start)
		{
		...
		public void Dispose(VoxelGenerator generator)
		{
			...
			{
				NativeVoxelTreeDisposer disposer;
				disposer.data = this;
				disposer.Schedule().Complete();
			...
			if (NodeQueue.IsCreated) NodeQueue.Dispose();
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static Fint3 ConvertLocalToInner(Ffloat3 localCoordinate, float rootSize)
		{
			return new Fint3(
				(int)((localCoordinate.x / rootSize) * INNERWIDTH),
				(int)((localCoordinate.y / rootSize) * INNERWIDTH),
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static int CalculateInnerOffset(int offset, int cellSubdivision)
        {		
			return offset * MathUtilities.DivideByPowerOf2(INNERWIDTH, cellSubdivision);
		}
	}
	...
	[BurstCompile]
    public unsafe struct NativeVoxelTreeDisposer : IJob
    {
		public NativeVoxelTree data;

        public void Execute()
        {
			NativeVoxelNode rootnode = UnsafeUtility.ReadArrayElement<NativeVoxelNode>(data._ROOT.ToPointer(), 0);
			rootnode.DestroyChildrenIgnoreID(ref data);
		}
    }
