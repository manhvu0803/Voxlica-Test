...
namespace Fraktalia.VoxelGen
{
	...
	[BurstCompile]
	public unsafe struct UpdateVoxelTree_Job : IJob
	{
		...
		public FNativeList<NativeVoxelNode> result_additive;
		...
		public FNativeList<NativeVoxelNode> result_additive_inner;
		...
		public void Init(VoxelGenerator generator)
		{
			...
			result_additive = new FNativeList<NativeVoxelNode>(Allocator.Persistent);
			...
			result_additive_inner = new FNativeList<NativeVoxelNode>(Allocator.Persistent);
		...
		public void ExecuteAdditive()
		{
			...
			for (int i = 0; i < TotalVoxels; i++)
			{
				...
				result_additive.Add(voxel);
			}

			int resultcount = result_additive.Length;
			for (int i = 0; i < resultcount; i++)
			{
				result_additive[i].Refresh(out voxel);
				if (!voxel.IsDestroyed())
				{
					data.UpwardMerge(voxel);
				}
			}
		...
		public void ExecuteAdditive_Inner()
		{
			...
			for (int i = 0; i < TotalVoxels; i++)
			{
				...
				voxel.SetVoxelAdditive(ref data, change.ID);
				result_additive_inner.Add(voxel);
			}

			int resultcount = result_additive_inner.Length;
			for (int i = 0; i < resultcount; i++)
			{
				result_additive_inner[i].Refresh(out voxel);
				if (!voxel.IsDestroyed())
				{
					data.UpwardMerge(voxel);
				}
		...
		public void CleanUp()
		{
			if (result_additive.IsCreated) result_additive.Dispose();
			...
			if (result_additive_inner.IsCreated) result_additive_inner.Dispose();
		...
		internal void SetCapacity(int count)
		{
			...
			result_additive.Capacity = count;
			...
			result_additive_inner.Capacity = count;
	...
	[BurstCompile]
	public unsafe struct NativeVoxelReset_Job : IJob
	{
		public NativeVoxelTree data;
