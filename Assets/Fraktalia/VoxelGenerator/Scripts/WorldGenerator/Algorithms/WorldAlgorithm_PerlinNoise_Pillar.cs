...
{
	public class WorldAlgorithm_PerlinNoise_Pillar : WorldAlgorithm
	{
		...
		WorldAlgorithm_PerlinNoise_Pillar_Calculate2D calculate;
		...
		public override JobHandle Apply(Vector3 hash, VoxelGenerator targetGenerator, ref JobHandle handle)
		{
			...
			calculate.Rotation = Quaternion.Euler(Rotation);
	...
	[BurstCompile]
	public struct WorldAlgorithm_PerlinNoise_Pillar_Calculate2D : IJobParallelFor
	{
		[ReadOnly]
		...
		public void Execute(int index)
		{
			...
			Vector3 position = new Vector3(Worldpos_X, Worldpos_Y, Worldpos_Z);
			position = Rotation * position;

			Worldpos_X = position.x;
			Worldpos_Y = position.y;
			Worldpos_Z = position.z;

			float distsqr = Worldpos_X * Worldpos_X + Worldpos_Y * Worldpos_Y;
			...
			if (Normalized == 1)
			{
				...
				float theta = 1;// (float)Mathf.Acos(Worldpos_Z / dist);
