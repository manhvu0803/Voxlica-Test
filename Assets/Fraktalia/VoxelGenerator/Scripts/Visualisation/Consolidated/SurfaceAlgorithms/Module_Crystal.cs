...
namespace Fraktalia.VoxelGen.Visualisation
{
	...
	public unsafe class Module_Crystal : HullGenerationModule
	{
		...
		void RunCompute(ComputeBuffer voxelsdata, float target, int lodwidth, int coreindex)
		{
            ...
            {
				int kernelindex = coreindex + 16;
				m_marchingCubes.SetBuffer(kernelindex, "mesh_verticeArray", mesh_verticeArray);
				m_marchingCubes.SetBuffer(kernelindex, "mesh_triangleArray", mesh_triangleArray);
				m_marchingCubes.SetBuffer(kernelindex, "mesh_normalArray", mesh_normalArray);
				m_marchingCubes.SetBuffer(kernelindex, "mesh_uvArray", mesh_uvArray);
				m_marchingCubes.SetBuffer(kernelindex, "PermutationBuffer", PermutationBuffer);
				m_marchingCubes.SetBuffer(kernelindex, "Voxels", voxelsdata);
				m_marchingCubes.SetBuffer(kernelindex, "_VertexNormalBuffer", _vertexnormaluvTriangleCombinedBuffer[coreindex]);
				m_marchingCubes.SetBuffer(kernelindex, "Counter", _counterBuffer[coreindex]);
				m_marchingCubes.SetBuffer(kernelindex, "PositionOffset", _positionOffsetBuffer[coreindex]);
				m_marchingCubes.Dispatch(kernelindex, lodwidth / 2, lodwidth / 2, lodwidth / 2);
			}
