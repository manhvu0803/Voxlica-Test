	...
	public unsafe class Module_MarchingCubes_GPU : HullGenerationModule
	{
		...
		public override IEnumerator beginCalculationasync(float cellSize, float voxelSize)
		{
			...
			for (int coreindex = 0; coreindex < HullGenerator.activeCores; coreindex++)
			{
				...
				RunCompute(voxeldata[coreindex], SurfacePoint+1, lodwidth, coreindex);
			}
		...
		void RunCompute(ComputeBuffer voxelsdata, float target, int lodwidth, int coreindex)
		{
            ...
            else
            {
				...
				m_marchingCubes.SetBuffer(kernelIndex, "counterBuffer", _counterBufferResult[coreindex]);
				m_marchingCubes.SetBuffer(kernelIndex, "TriangleTable", _triangleTable);
				m_marchingCubes.SetBuffer(kernelIndex, "Voxels", voxelsdata);
				m_marchingCubes.SetBuffer(kernelIndex, "_VertexNormalBuffer", _vertexnormalCombinedBuffer[coreindex]);
