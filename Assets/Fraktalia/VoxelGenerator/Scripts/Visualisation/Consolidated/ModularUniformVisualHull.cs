...
namespace Fraktalia.VoxelGen.Visualisation
{
	public class NativeMeshData
    {
		...
		public FNativeList<Vector4> uv2Array;
		...
		public FNativeList<Vector4> uv7Array;
		public FNativeList<Vector4> uv8Array;
		...
		public void Initialize()
        {
			...
			uv2Array = new FNativeList<Vector4>(0, Allocator.Persistent);
			uv3Array = new FNativeList<Vector4>(0, Allocator.Persistent);
			uv4Array = new FNativeList<Vector4>(0, Allocator.Persistent);
			uv5Array = new FNativeList<Vector4>(0, Allocator.Persistent);
			uv6Array = new FNativeList<Vector4>(0, Allocator.Persistent);
			uv7Array = new FNativeList<Vector4>(0, Allocator.Persistent);
			uv8Array = new FNativeList<Vector4>(0, Allocator.Persistent);
		...
		public void CleanUp()
		{
			...
			if (uv2Array.IsCreated) uv2Array.Dispose();
			...
			if (uv7Array.IsCreated) uv7Array.Dispose();
			if (uv8Array.IsCreated) uv8Array.Dispose();
	...
	public class ModularUniformVisualHull : BasicNativeVisualHull
	{
		...
		[PropertyKey("LOD Settings", false)][ReadonlyText()]
        ...
        [PropertyModule][SerializeField] private Module_MarchingCubes_CPU moduleMarchingCubes_CPU;
        [PropertyModule][SerializeField] private Module_MarchingCubes_GPU moduleMarchingCubes_GPU;
		...
		NativeCreateUniformGrid_V3[] calculators;
		...
		protected override void Initialize()
		{
			...
			switch (MeshSurfaceAlgorithm)
            {
                case SurfaceAlgorithm.None:
					...
					break;
                case SurfaceAlgorithm.MarchingCubesGPU:
					currentSurfaceModule = moduleMarchingCubes_GPU;
					break;
				...
				case SurfaceAlgorithm.Mesh:
					...
					break;
                case SurfaceAlgorithm.MarchingCubesCPU:
                    currentSurfaceModule = moduleMarchingCubes_CPU;
                    break;
                default:
                    break;
            }
			...
			CurrentLOD = Mathf.Clamp(CurrentLOD, 0, LODSize.Count - 1);
			#endregion
			...
			calculators = new NativeCreateUniformGrid_V3[CurrentNumCores];
			for (int i = 0; i < CurrentNumCores; i++)
            {
				...
				calculators[i] = new NativeCreateUniformGrid_V3();
        ...
        {
			int lod = Mathf.Clamp(CurrentLOD, 0, LODSize.Count - 1);
			...
			for (int cores = 0; cores < activeCores; cores++)
			{
				...
				calculators[cores].ShrinkMax = 0;

				if ((int)MeshSurfaceAlgorithm >= 3)
					calculators[cores].ShrinkMax = Shrink << MathUtilities.RightmostBitPosition(innerVoxelSize);
		...
		private void finishCalculation()
		{
            for (int i = 0; i < activeCores; i++)
            {
				...
				if (data.verticeArray.Length != 0)
				{
					...
					if (data.uv2Array.Length == 0 || data.uv2Array.Length == data.verticeArray.Length)
						piece.SetUVs(1, data.uv2Array);
					...
					if (data.uv7Array.Length == 0 || data.uv7Array.Length == data.verticeArray.Length)
						piece.SetUVs(6, data.uv7Array);

					if (data.uv8Array.Length == 0 || data.uv8Array.Length == data.verticeArray.Length)
						piece.SetUVs(7, data.uv8Array);
						...
						for (int z = cellRanges_min.z; z <= cellRanges_max.z; z++)
						{
							...
							{								
								if ((int)WorkInformations[index].CurrentWorktype < (int)requiredWorktype)
								{
									...
									//Hotfix but inefficient (Contains = O(N))
									if(!WorkerQueue.Contains(index))
                                    {
										WorkerQueue.Enqueue(index);										
                                    }
		...
		public override void OnInspectorGUI()
		{
			...
			switch (mytarget.MeshSurfaceAlgorithm)
            {
                case SurfaceAlgorithm.None:
                    break;
                case SurfaceAlgorithm.MarchingCubesGPU:
					EditorGUILayout.PropertyField(serializedObject.FindProperty("moduleMarchingCubes_GPU"), true);
					break;
                case SurfaceAlgorithm.Block:
				...
				case SurfaceAlgorithm.Mesh:
					...
					break;
                case SurfaceAlgorithm.MarchingCubesCPU:
                    EditorGUILayout.PropertyField(serializedObject.FindProperty("moduleMarchingCubes_CPU"), true);
                    break;
                default:
                    break;
            }
			...
			if (selectalgorithm > 0)
			{
				...
				NewPostProcess.transform.localPosition = Vector3.zero;
				NewPostProcess.transform.localRotation = Quaternion.identity;
			...
			if (GUI.changed)
			{
				...
				{
					var myTarget = item as ModularUniformVisualHull;
