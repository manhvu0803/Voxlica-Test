...
namespace Fraktalia.VoxelGen.Visualisation
{
	...
	[BurstCompile]
	public struct MarchingCubes_Transvoxel_Calculation : IJob
	{
		...
		[BurstDiscard]
		public void Init(float surface = 0.5f)
		{
			CleanUp();
			...
			uv3Array = new FNativeList<Vector2>(2000, Allocator.Persistent);
			uv4Array = new FNativeList<Vector2>(2000, Allocator.Persistent);
			uv5Array = new FNativeList<Vector2>(2000, Allocator.Persistent);
			uv6Array = new FNativeList<Vector2>(2000, Allocator.Persistent);
			...
			colorArray = new FNativeList<Color>(2000, Allocator.Persistent);
		}

		public void SetWidth(int width)
		{
			Width = width;
			MaxBlocks = Width * Width * Width;
		}
			...
			for (int index = 0; index < blocks; index++)
			{
				...
				for (i = 0; i < 5; i++)
				{
					...
					for (j = 0; j < 3; j++)
					{
						...
						{
							float texturevalue = texturedata_UV4._PeekVoxelId(vertex.x, vertex.y, vertex.z, 10, Shrink);
							uv4Array.Add(new Vector2(texturevalue, texturevalue));
						}

						if (texturedata_UV5.IsCreated)
						{
							float texturevalue = texturedata_UV5._PeekVoxelId(vertex.x, vertex.y, vertex.z, 10, Shrink);
							uv5Array.Add(new Vector2(texturevalue, texturevalue));
						}

						if (texturedata_UV6.IsCreated)
						{
							float texturevalue = texturedata_UV6._PeekVoxelId(vertex.x, vertex.y, vertex.z, 10, Shrink);
