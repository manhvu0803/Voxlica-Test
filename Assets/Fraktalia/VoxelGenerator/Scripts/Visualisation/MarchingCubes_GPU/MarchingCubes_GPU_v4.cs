...
{
	[Obsolete("This hull generator is obsolete! Use ModularUniformVisualHull instead. Its faster and provides much more flexibility!")]
	public unsafe class MarchingCubes_GPU_v4 : UniformGridVisualHull_SingleStep_Async
	{
		...
		protected override void initializeCalculators()
		{
			...
			readbackRequests = new AsyncGPUReadbackRequest[NumCores];
		...
		protected override IEnumerator beginCalculationasync(Queue<int> WorkerQueue, float cellSize, float voxelSize)
		{
			...
			for (int coreindex = 0; coreindex < works; coreindex++)
			{
				...
				readbackRequests[coreindex] = AsyncGPUReadback.RequestIntoNativeArray(ref verticesnormalsCombined[coreindex], _vertexnormalCombinedBuffer[coreindex]);
			}
			...
			for (int coreindex = 0; coreindex < works; coreindex++)
			{
				...
				AsyncGPUReadbackRequest requestverts = readbackRequests[coreindex];
