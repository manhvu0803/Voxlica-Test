...
namespace Fraktalia.VoxelGen.Visualisation
{
	public class Detail : BasicNativeVisualHull
	{
		...
		public override void RootNodeChanged(NativeVoxelNode node)
		{
		...
		public override void NodesChanged(ref UpdateVoxelTree_Job worker)
		{
			if (worker.result_additive.Length > 0)
			{
				nodechangedjob.voxels = worker.result_additive;
				JobHandle job = nodechangedjob.Schedule(worker.result_additive.Length, worker.result_additive.Length / NumCores);
				job.Complete();
			}

			if (worker.result_additive_inner.Length > 0)
			{
				nodechangedjob.voxels = worker.result_additive_inner;
				JobHandle job = nodechangedjob.Schedule(worker.result_additive_inner.Length, worker.result_additive_inner.Length / NumCores);
				job.Complete();
			}

			if (worker.result_set.Length > 0)
			{
				nodechangedjob.voxels = worker.result_set;
				JobHandle job = nodechangedjob.Schedule(worker.result_set.Length, worker.result_set.Length / NumCores);
				job.Complete();
			}

			if (worker.result_set_inner.Length > 0)
			{
				nodechangedjob.voxels = worker.result_set_inner;
				JobHandle job = nodechangedjob.Schedule(worker.result_set_inner.Length, worker.result_set_inner.Length / NumCores);
				job.Complete();
			}
