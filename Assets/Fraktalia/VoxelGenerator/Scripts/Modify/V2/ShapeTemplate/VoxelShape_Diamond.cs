...
namespace Fraktalia.VoxelGen.Modify
{
	...
	[System.Serializable]
	public class VoxelShape_Diamond : VoxelShape_Base
	{
		public Vector3 Rotation;


		public int InitialID = 500;
		...
		{
			Vector3 scaledBounds = Bounds * modifier.ShapeScale;
			float scaledRoundness = Roundness * modifier.ShapeScale;
			...
			DiamondCalculationJob job = new DiamondCalculationJob();
		...
		public override bool RequiresRecalculation(VoxelModifier_V2 modifier, VoxelGenerator target)
		{
			...
			float sum = scaledBounds.sqrMagnitude + scaledRoundness + InitialID + Rotation.sqrMagnitude + (ApplyObjectRotation ? 0 : 1) + boundaryExtension;
	...
	[BurstCompile]
	public struct DiamondCalculationJob : IJobParallelFor
	{
		...
		{
			float Dist_X = sabs(Pos_X);
			float Dist_Y = sabs(Pos_Y);
			float Dist_Z = sabs(Pos_Z);

			float Factor_X = (Dist_X) / (BoxSize.x);
			float Factor_Y = (Dist_Y) / (BoxSize.y);
			float Factor_Z = (Dist_Z) / (BoxSize.z);

			return Mathf.Max(0, (Factor_X + Factor_Y + Factor_Z) * 1);
		}

		//Function which creates the roundness for round edges.
		private float sabs(float p) => Mathf.Sqrt((p) * (p) + Mathf.Max(0, Roundness));
	}
}
