...
namespace Fraktalia.VoxelGen.Modify
{
	...
	public class VoxelShape_Box : VoxelShape_Base
	{
		...
		public Vector3 Radials = new Vector3(4,4,4);
		...
		public override void DrawEditorPreview(VoxelModifier_V2 modifier, bool isSafe, Vector3 worldPosition, Vector3 normal)
		{
			...
			Gizmos.DrawWireCube(Vector3.zero, scaledBounds);
		}
	
		protected override void calculateTemplateData(VoxelModifier_V2 modifier, VoxelGenerator target)
		{
			Vector3 scaledBounds = Bounds * modifier.ShapeScale;
			Vector3 scaledRadials = Radials * modifier.ShapeScale;
			...
			scaledRadials.x = Mathf.Max(0.01f, scaledRadials.x);
			scaledRadials.y = Mathf.Max(0.01f, scaledRadials.y);
			scaledRadials.z = Mathf.Max(0.01f, scaledRadials.z);

			BoxCalculationJob job = new BoxCalculationJob();
			job.Rotation = Quaternion.Inverse(rot);
			job.BoxSize = scaledBounds / 2;
			job.Radius_X = scaledRadials.x;
			job.Radius_Y = scaledRadials.y;
			job.Radius_Z = scaledRadials.z;
		...
		public override Vector3 GetGameIndicatorSize(VoxelModifier_V2 modifier)
		{
			...
			Quaternion rot = calculateRotation(Rotation, modifier);
			return MathUtilities.RotateBoundary(scaledBounds, rot);	
		}
		...
		public override bool RequiresRecalculation(VoxelModifier_V2 modifier, VoxelGenerator target)
		{
			...
			float sum = scaledBounds.sqrMagnitude + scaledRadials.sqrMagnitude + InitialID + Rotation.sqrMagnitude + (ApplyObjectRotation ? 0 : 1) + boundaryExtension;
