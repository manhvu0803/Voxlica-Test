	...
	public class VoxelShape_Sphere : VoxelShape_Base
	{
		...
		public override void DrawEditorPreview(VoxelModifier_V2 modifier, bool isSafe, Vector3 worldPosition, Vector3 normal)
		{
			...
			for (int i = 0; i < 8; i++)
            {
				...
				Gizmos.DrawWireCube(Vector3.zero, new Vector3(scaledradius * 2, 0, scaledradius * 2));
		...
		public override Vector3 GetGameIndicatorSize(VoxelModifier_V2 modifier)
		{
			...
			return new Vector3(scaledradius * 2, scaledradius * 2, scaledradius * 2);
		}
		...
		protected override void calculateTemplateData(VoxelModifier_V2 modifier, VoxelGenerator target)
		{
			...
			int rows = (int)(scaledradius * 2 / voxelsize) + boundaryExtension;
			rows += rows % 2;
		...
		public override void SetGeneratorDirty(VoxelModifier_V2 modifier, VoxelGenerator target, Vector3 worldPosition)
		{
			...
			target.SetRegionsDirty(target.transform.worldToLocalMatrix.MultiplyPoint3x4(worldPosition), Vector3.one * (scaledradius + voxelsize * boundaryExtension/2), Vector3.one * (scaledradius + voxelsize * boundaryExtension / 2), modifier.TargetDimension);
		}
        ...
        {
			float sum = Radius * modifier.ShapeScale + (ApplyObjectRotation ? 0 : 1) + InitialID + boundaryExtension;
