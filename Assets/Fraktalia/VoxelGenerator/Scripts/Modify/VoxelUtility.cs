...
namespace Fraktalia.VoxelGen.Modify
{
	...
	public static class VoxelUtility_V2
    {
		public static void GatherVoxelInformation(ref NativeVoxelTree data, ref FNativeList<NativeVoxelModificationData_Inner> voxelsToRead, ref FNativeList<NativeVoxelModificationData_Inner> result)
        {
			if(voxelsToRead.Length != result.Length)
            {
				result.Resize(voxelsToRead.Length, NativeArrayOptions.UninitializedMemory);
            }

			GatherInformationJob gatherpredata = new GatherInformationJob();
			gatherpredata.data = data;
			gatherpredata.readvoxeldata = voxelsToRead;
			gatherpredata.resultvoxeldata = result;
			gatherpredata.Schedule(voxelsToRead.Length, voxelsToRead.Length / SystemInfo.processorCount).Complete();
		}

		public static void RepositionVoxelInformation(Vector3Int offset, ref FNativeList<NativeVoxelModificationData_Inner> input, ref FNativeList<NativeVoxelModificationData_Inner> output)
        {
			if (input.Length != output.Length)
			{
				output.Resize(input.Length, NativeArrayOptions.UninitializedMemory);
			}

			RepositionJob reposition;
			reposition.template = input;
			reposition.results = output;
			reposition.Offset_X = offset.x;
			reposition.Offset_Y = offset.y;
			reposition.Offset_Z = offset.z;
			reposition.Schedule(input.Length, input.Length / SystemInfo.processorCount).Complete();
		}

		public static void DrawVoxelGeneratorBoundary(VoxelGenerator generator)
        {
            ...
            {
				Gizmos.color = new Color32(64, 64, 255, 180);

				if (!generator.IsIdle)
				{
					Gizmos.color = new Color32(100,100,255,255);
				}
				
            }
	
			Gizmos.matrix = generator.transform.localToWorldMatrix;
			float RootSize = generator.RootSize;
			Vector3 center = new Vector3(RootSize, RootSize, RootSize) / 2;
			Gizmos.DrawWireCube(center, new Vector3(RootSize, RootSize, RootSize));
			RootSize *= 0.999f;
			Gizmos.DrawWireCube(center, new Vector3(RootSize, RootSize, RootSize));
			RootSize *= 0.999f;
			Gizmos.DrawWireCube(center, new Vector3(RootSize, RootSize, RootSize));
		...
		public static void DrawVoxelGeneratorBoundary(VoxelGenerator generator, Color32 color, float sizemultiplier = 1)
		{
			Gizmos.matrix = generator.transform.localToWorldMatrix;
			float RootSize = generator.RootSize;
			Vector3 center = new Vector3(RootSize, RootSize, RootSize) / 2;
			RootSize *= sizemultiplier;
			Gizmos.color = color;
			Gizmos.DrawWireCube(center, new Vector3(RootSize, RootSize, RootSize));
		}
	}
