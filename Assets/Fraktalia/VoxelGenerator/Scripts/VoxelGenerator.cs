...
namespace Fraktalia.VoxelGen
{
	...
	{
		public const int MINCAPACITY = 10000;
		public const int MAXVOXELMODIFY = 25165824;
		...
		public NativeVoxelTree[] Data = new NativeVoxelTree[0];

		[Range(1, 16)] [Tooltip("How many separate voxel data. First one is usually the density which is used for geometry generation, second one can be used for texture value or other usages")]
		...
		[Tooltip("Additional initial values for dimension 1-X since InitialValue is only for dimension 0.")]
		public int[] AdditionalInitialValues = new int[0];
		...
		private Queue<VoxelRegion> regionstoupdate = new Queue<VoxelRegion>();
		...
		public static Dictionary<int, VoxelGenerator> ActiveGenerators = new Dictionary<int, VoxelGenerator>();
		...
		public void GenerateBlock()
		{
			...
			{
				int initialvalue = 0;
				if (AdditionalInitialValues != null)
				{
					if (i - 1 < AdditionalInitialValues.Length) initialvalue = AdditionalInitialValues[i - 1];
				}
				Data[i]._Initialize(memoryReservoir[i], RootSize, SubdivisionPower, (byte)initialvalue, 64);
			}
			...
			for (int i = 0; i < hullGenerators.Length; i++)
			{
				hullGenerators[i].RootNodeChanged(root);
			}
			...
			ActiveGenerators.Add(this.GetInstanceID(), this);
		}
		...
		//Kill an entire dimension (set to 0)
		public void KillDimension(int dimension)
        {
			ResetDimension(dimension, 0);
        }

		//Fill an entire dimension (set to 255)
		public void FillDimension(int dimension)
		{
			ResetDimension(dimension, 255);
		}

		public void _SetVoxel(Vector3 localposition, byte depth, byte id, int dimension = 0)
		{
			if (RestrictDimension[dimension]) return;
			if (setadditivejob[dimension].changedata_set.Length > MAXVOXELMODIFY)
			{
				UnityEngine.Debug.LogError("Voxel Modification Cap exceeded! Request would exceed maximum of " + MAXVOXELMODIFY + " simultaneous modifications.");
				return;
			}
		...
		public void _SetVoxels(List<Vector3> localposition, List<byte> depth, List<int> id, int dimension = 0)
		{
			...
			if (setadditivejob[dimension].changedata_set.Length + localposition.Count > MAXVOXELMODIFY)
			{
				UnityEngine.Debug.LogError("Voxel Modification Cap exceeded! Request would exceed maximum of " + MAXVOXELMODIFY + " simultaneous modifications.");
				return;
			}
		...
		public void _SetVoxels(NativeVoxelModificationData[] rawvoxeldata, int dimension = 0)
		{
			...
			if (setadditivejob[dimension].changedata_set.Length + rawvoxeldata.Length > MAXVOXELMODIFY)
			{
				UnityEngine.Debug.LogError("Voxel Modification Cap exceeded! Request would exceed maximum of " + MAXVOXELMODIFY + " simultaneous modifications.");
				return;
			}
		...
		public unsafe void _SetVoxels(byte[] rawvoxeldata, int voxelcount, int dimension = 0)
		{
			...
			if (setadditivejob[dimension].changedata_set.Length + voxelcount > MAXVOXELMODIFY)
			{
				UnityEngine.Debug.LogError("Voxel Modification Cap exceeded! Request would exceed maximum of " + MAXVOXELMODIFY + " simultaneous modifications.");
				return;
			}
		...
		{
			if (RestrictDimension[dimension]) return;
			if (setadditivejob[dimension].changedata_set_inner.Length + rawvoxeldata.Length > MAXVOXELMODIFY)
			{
				UnityEngine.Debug.LogError("Voxel Modification Cap exceeded! Request would exceed maximum of " + MAXVOXELMODIFY + " simultaneous modifications.");
				return;
			}
		...
		{
			if (RestrictDimension[dimension]) return;
			if (setadditivejob[dimension].changedata_set_inner.Length + rawvoxeldata.Count > MAXVOXELMODIFY)
			{
				UnityEngine.Debug.LogError("Voxel Modification Cap exceeded! Request would exceed maximum of " + MAXVOXELMODIFY + " simultaneous modifications.");
				return;
			}
		...
		{
			if (RestrictDimension[dimension]) return;
			if (setadditivejob[dimension].changedata_additive.Length> MAXVOXELMODIFY)
			{
				UnityEngine.Debug.LogError("Voxel Modification Cap exceeded! Request would exceed maximum of " + MAXVOXELMODIFY + " simultaneous modifications.");
				return;
			}
		...
		public void _SetVoxelsAdditive(List<NativeVoxelModificationData> rawvoxeldata, int dimension = 0)
		{
			if (RestrictDimension[dimension]) return;
			if (setadditivejob[dimension].changedata_additive.Length + rawvoxeldata.Count > MAXVOXELMODIFY)
			{
				UnityEngine.Debug.LogError("Voxel Modification Cap exceeded! Request would exceed maximum of " + MAXVOXELMODIFY + " simultaneous modifications.");
				return;
			}
			...
			for (int i = 0; i < rawvoxeldata.Count; i++)
			{
				setadditivejob[dimension].changedata_additive.Add(rawvoxeldata[i]);
			}
			VoxelTreeDirty = true;
		}
		...
		{
			if (RestrictDimension[dimension]) return;
			if (setadditivejob[dimension].changedata_additive.Length + rawvoxeldata.Length > MAXVOXELMODIFY)
			{
				UnityEngine.Debug.LogError("Voxel Modification Cap exceeded! Request would exceed maximum of " + MAXVOXELMODIFY + " simultaneous modifications.");
				return;
			}
		...
		{
			if (RestrictDimension[dimension]) return;
			if (setadditivejob[dimension].changedata_additive_inner.Length + rawvoxeldata.Length > MAXVOXELMODIFY)
			{
				UnityEngine.Debug.LogError("Voxel Modification Cap exceeded! Request would exceed maximum of " + MAXVOXELMODIFY + " simultaneous modifications.");
				return;
			}
		...
		{
			if (RestrictDimension[dimension]) return;
			if (setadditivejob[dimension].changedata_additive_inner.Length + rawvoxeldata.Count > MAXVOXELMODIFY)
			{
				UnityEngine.Debug.LogError("Voxel Modification Cap exceeded! Request would exceed maximum of " + MAXVOXELMODIFY + " simultaneous modifications.");
				return;
			}
		...
		{
			Vector3Int inner = ConvertWorldToInner(worldPosition, transform.worldToLocalMatrix, RootSize);
			return Data[dimension]._PeekVoxelId_InnerCoordinate(inner.x, inner.y, inner.z, 16, shrink, border);
		}

		/// <summary>
		/// Efficient way to read voxel values. Output Array can be provided in order to prevent allocation.
		/// </summary>
		/// <param name="worldPositions">World positions</param>
		/// <param name="dimension">Target dimension</param>
		/// <param name="shrink">Remove voxels at the edge to prevent "open" results.</param>
		/// <param name="border">Value used if position is outside volume.</param>
		/// <param name="outputValues">If null, result is allocated. Optionally provide array with same size as worldPositions</param>
		/// <returns></returns>
		public int[] GetVoxelValuesAt(Vector3[] worldPositions, int dimension, int shrink = 0, int border = 0, int[] outputValues = null)
		{
			int count = worldPositions.Length;
			if (outputValues == null || outputValues.Length != count)
			{
				outputValues = new int[count];
			}

			var job = new GetVoxelValuesJob
			{
				worldPositions = new NativeArray<Vector3>(worldPositions, Allocator.TempJob),
				dimension = dimension,
				shrink = shrink,
				border = border,
				outputValues = new NativeArray<int>(count, Allocator.TempJob),
			...
			job.outputValues.CopyTo(outputValues);			
			job.worldPositions.Dispose();
			job.outputValues.Dispose();

			return outputValues;
		}
		#endregion

		#region MultiBlock and Neighbors				
		[NonSerialized]
		public VoxelGenerator[] Neighbours = new VoxelGenerator[27];

		public void SetNeighbor(int ID, VoxelGenerator neighbor)
		{
			if (ID == 13) return;
			if (!neighbor.IsInitialized) return;
			...
			while (IsInitialized)
			{
				...
				for (int i = 0; i < Data.Length; i++)
				{
					...
					if (jobHandlesProcessing[i])
					{
						...
						for (int k = 0; k < hullGenerators.Length; k++)
						{							
							hullGenerators[k].NodesChanged(ref updatevoxeltree);
						}

						updatevoxeltree.result_additive.Clear();
						updatevoxeltree.result_set.Clear();
						updatevoxeltree.result_additive_inner.Clear();
						updatevoxeltree.result_set_inner.Clear();

						if (MemoryOptimized)
						{
							updatevoxeltree.result_additive.Capacity = MINCAPACITY;
							updatevoxeltree.result_set.Capacity = MINCAPACITY;
							updatevoxeltree.result_additive_inner.Capacity = MINCAPACITY;
		...
		public IEnumerator updateHulls()
		{
			...
			while (ContainerStaticLibrary.AsyncLock && IsInitialized)
			{
			...
			ContainerStaticLibrary.AsyncLock = true;
			for (int i = 0; i < hullGenerators.Length; i++)
			{
			...
			ContainerStaticLibrary.AsyncLock = false;
		...
		{
			VoxelUtility_V2.DrawVoxelGeneratorBoundary(this);						
			hullGenerators = GetComponentsInChildren<BasicNativeVisualHull>();			
		}
		...
		private void EditorUpdate()
        {
			...
			if (IsInitialized && !Application.isPlaying && isActiveAndEnabled)
			{
				Update();
			}
				...
				if (instanceID != gameObject.GetInstanceID())
				{
					...
					try
					{
						...
						if (original != null)
						{
							...
							CleanUp();
							originalgenerator.CleanUp();
					
							VoxelSaveSystem oldsave = originalgenerator.savesystem;
							VoxelSaveSystem newsave = GetComponent<VoxelSaveSystem>();
							if (oldsave && oldsave.ModuleScriptableObject._VoxelMap != null)
							{
								...
								if (oldsave.EditorSaveMode == EditorVoxelSaveMode.ScriptableObject && oldsave.ModuleScriptableObject.DuplicateMapOnClone)
								{
									...
									newsave.ModuleScriptableObject._VoxelMap = AssetDatabase.LoadAssetAtPath<VoxelMap>(newpath);
									oldsave.Save();
								}
							}

							if (newsave) newsave.hasloaded = true;

							WorldGenerator oldworld = originalgenerator.worldgenerator;
							WorldGenerator newworld = GetComponent<WorldGenerator>();
												
							VoxelSaveSystem.ApplyRawVoxelData(this, olddata);
							VoxelSaveSystem.ApplyRawVoxelData(originalgenerator, olddata);
						}
					}
                    catch
                    {
		...
		public static void CleanUpStatics()
		{
			...
			NativeVoxelReservoir.CleanReservoirs();
		...
		public void CleanUp()
		{
			...
			ActiveGenerators.Remove(GetInstanceID());
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static Vector3Int ConvertWorldToInner(Vector3 worldCoordinate, Matrix4x4 worldToLocalMatrix, float rootSize)
		{
			Vector3 localCoordinate = worldToLocalMatrix.MultiplyPoint3x4(worldCoordinate);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static Vector3 ConvertInnerToWorld(Vector3Int innerCoordinate, Matrix4x4 localToWorldMatrix, float rootSize)
		{
			return localToWorldMatrix.MultiplyPoint3x4( new Vector3(
				(float)innerCoordinate.x / (float)NativeVoxelTree.INNERWIDTH * rootSize,
				(float)innerCoordinate.y / (float)NativeVoxelTree.INNERWIDTH * rootSize,
				(float)innerCoordinate.z / (float)NativeVoxelTree.INNERWIDTH * rootSize));
		}
		...
		public void ClearRegionUpdate() => regionstoupdate.Clear();
		#endregion
	}
	...
	[BurstCompile]
    struct GetVoxelValuesJob : IJobParallelFor
    {
		[ReadOnly]
		...
		[WriteOnly]
		public NativeArray<int> outputValues;
		...
		{
			Vector3Int inner = VoxelGenerator.ConvertWorldToInner(worldPositions[i], worldToLocalMatrix, rootSize);
			outputValues[i] = data._PeekVoxelId_InnerCoordinate(inner.x, inner.y, inner.z, 16, shrink, border);
		}	
	}
		...
		public override void OnInspectorGUI()
		{
			...
			if (!myTarget.Locked)
			{
				...
				if (GUILayout.Button(new GUIContent("Generate Block ID 0", "Fills the entire volume with initialValue 0.")))
				{
                    ...
                    {
						var currentTaget = targets[i] as VoxelGenerator;
						currentTaget.InitialValue = 0;
						currentTaget.GenerateBlock();
                    }
				}
				...
				if (myTarget.worldgenerator)
				{
					if (GUILayout.Button(new GUIContent("Generate World", "Fills the entire volume with the calculation from all the attached World Algoriths.")))
					{
						...
						{
							var currentTaget = targets[i] as VoxelGenerator;
							if(currentTaget.worldgenerator)
								currentTaget.worldgenerator.InitializeGeneratorAndApplyWorld();
						}	
					}
				}
				...
				if(myTarget.savesystem)
                {
					...
					if (GUILayout.Button(new GUIContent("Save Data")))
					{
						...
						{
							var currentTaget = targets[i] as VoxelGenerator;
							if (currentTaget.savesystem)
								currentTaget.savesystem.Save();
						}
					}
			...
			if (!Application.isPlaying)
			{
				...
				if (sv != null)
				{
					if (!sv.drawGizmos)
					{
						...
						EditorGUILayout.TextArea("Many functionalities of Voxelica require Gizmos being enabled!");
