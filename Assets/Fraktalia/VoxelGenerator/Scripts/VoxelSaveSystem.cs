...
namespace Fraktalia.VoxelGen
{
	...
	public class VoxelSaveSystem : MonoBehaviour
	{
		...
		[Tooltip("When true, data is saved when scene saves. Is true by default.")]
		public bool SaveOnSceneSave = true;
...
#if UNITY_EDITOR
			__migrated = true;
			if (ModuleScene != null) ModuleScene.SceneVoxelData = SceneVoxelData;

			if (ModuleScriptableObject != null) ModuleScriptableObject._VoxelMap = _VoxelMap;
			if (ModuleScriptableObject != null) ModuleScriptableObject.SaveInSceneSubFolder = SaveInSceneSubFolder;
			if (ModuleScriptableObject != null) ModuleScriptableObject.RemoveVoxelmapOnDelete = RemoveVoxelmapOnDelete;
			if (ModuleScriptableObject != null) ModuleScriptableObject.DuplicateMapOnClone = DuplicateMapOnClone;

			if (ModulePersistent != null) ModulePersistent.VoxelName = VoxelName;

			if (ModuleWorld != null) ModuleWorld.VoxelName = VoxelName;
			if (ModuleWorld != null) ModuleWorld.WorldFolder = WorldFolder;
			if (ModuleWorld != null) ModuleWorld.WorldChunk = WorldChunk;
			if (ModuleWorld != null) ModuleWorld.WorldToSceneLocation = WorldToSceneLocation;
			if (ModuleWorld != null) ModuleWorld.WorldLoadingRule = WorldLoadingRule;

			if (ModuleRegion != null) ModuleRegion.WorldRegionSize = WorldRegionSize;
			if (ModuleRegion != null) ModuleRegion.VoxelName = VoxelName;
			if (ModuleRegion != null) ModuleRegion.WorldFolder = WorldFolder;
			if (ModuleRegion != null) ModuleRegion.WorldChunk = WorldChunk;
			if (ModuleRegion != null) ModuleRegion.WorldToSceneLocation = WorldToSceneLocation;
			if (ModuleRegion != null) ModuleRegion.WorldLoadingRule = WorldLoadingRule;
	...
	public class VoxelSaveSystemSaver : UnityEditor.AssetModificationProcessor
	{
		static string[] OnWillSaveAssets(string[] paths)
		{
			...
			for (int i = 0; i < voxelengines.Length; i++)
			{
				...
				{
					if(voxelengines[i].SaveOnSceneSave)
					voxelengines[i].Save();
				}
			}
			return paths;
	...
	public class VoxelSaveSystemEditor : Editor
	{
		public override void OnInspectorGUI()
		{
			...
			EditorGUILayout.PropertyField(serializedObject.FindProperty("SaveOnSceneSave"), new GUIContent("Save on scene save"));
			...
			{			
				...
				{
					var currentTaget = targets[i] as VoxelSaveSystem;
					currentTaget.IsDirty = true;
					currentTaget.Save();
				}
			}
